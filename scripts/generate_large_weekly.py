#!/usr/bin/env python3
"""My Leisure Time Weekly Report: Discord data + ARK deep analysis."""
import json, os, datetime, urllib.request, collections, sys

TOKEN = os.environ.get("DISCORD_BOT_TOKEN", "")
ARK_KEY = os.environ.get("ARK_API_KEY", "")
FEISHU = os.environ.get("FEISHU_LARGE_WEBHOOK", "")
SUPABASE_URL = "https://rryzofimrehmkijkckrm.supabase.co"
SUPABASE_KEY = "sb_publishable_oyewqnQ8AnitAOD94Qg0nA_v6Zqkr7r"
CACHE_FILE = "large_weekly_cache.json"

GUILD_ID = "1427259763334451220"
CHANNELS = {
    "chat-area": "1427632605616144428",
    "game-chat": "1427633748088123443",
    "fan-creations": "1454017168445542450",
    "newbie-help": "1427632515417509939",
    "strategy-chat": "1427633890425385001",
    "pet-paradise": "1428156536141316268",
    "cozy-kitchen": "1462713125577949220",
    "find-a-friend": "1453086547967606854",
    "inspiration": "1462724384778227920",
    "design-request": "1454011477542437075",
}
...
def arkanalyze(messages):
    ...
    "content_categories":[],"keyword_cloud":[],"weekly_summary":"",}
    text = "\n".join(f"{i+1}. {m}" for i,m in enumerate(meaningful[:80]))
    prompt = f"""你是 My Leisure Time 游戏社群的运营分析师。仔细阅读本周 Discord 聊天记录。

社群背景：这是一个大型游戏玩家社群（3万+人），包含聊天、创作、攻略、求助、宠物等多个频道。运营需要了解玩家在聊什么、抱怨什么、喜欢什么。

请深度分析（周报数据有限，每条消息都要深挖）：

1. **热议话题深层解读**：具体聊什么内容？有什么分歧或共识？这个话题为什么重要？谁在主导讨论？对社群有什么潜在影响？
2. **玩家痛点**：具体抱怨什么游戏机制、流程、体验？不只是说某个功能不好，要说清楚哪里不好、影响了谁、有多严重。
3. **游戏内容讨论**：玩家在讨论哪些游戏内容？创作攻略？设计比赛？新功能期待？
4. **新人体验**：新来的玩家遇到什么困难？有没有被帮助？有没有沉默或流失的迹象？
5. **社群氛围**：互助行为有哪些？有没有矛盾或争吵？氛围向上还是向下？
6. **舆情监控**：有没有负面情绪信号（如 boring, bug, dead, quit 等关键词）？有没有对官方的不满？
7. **运营洞察**：你觉得这批数据里，运营最该关注什么？
        ...
        if item.get("type") == "message":
            for c in item.get("content",[]):
                if c.get("type") == "output_text":
                    ...
                    except: return {"hot_discussions":[],"user_sentiment":c.get("text","")[:200],"pain_points":[],"highlights":[],"notable_quotes":[],"emerging_topics":"","content_categories":[],"keyword_cloud":[],"weekly_summary":"",}
    return {"hot_discussions":[],"user_sentiment":"分析失败","pain_points":[],"highlights":[],"notable_quotes":[],"emerging_topics":"","content_categories":[],"keyword_cloud":[],"weekly_summary":"",}
...
def main():
    ...
    print(f"📊 My Leisure Time 周报 · {week_label}")

    # Step 1: Main channel full data
    print("📡 采集 chat-area...")
    mc, main_speakers, daily, user_rank, samples = count_week_full(CHANNELS["chat-area"])
    ...
    if ARK_KEY and all_samples:
        print("\n🧠 第二轮 ARK: 运营分析...")
        strat_prompt = "你是游戏社群的运营分析师。基于本周聊天数据，用中文写一个200字以上的运营总结，必须包含三段：\n\n【问题诊断】列出1-2个核心问题及影响\n【行动建议】给出2-3条可执行动作+预期效果\n【路线图】本周做什么→两周内做什么→一个月内达成什么\n\n聊天数据：\n" + "\n".join(all_samples[:40])
...
<title>My Leisure Time · 周报 {week_label}</title>
<style>
...
<div class="header">
  ...
  <h1>My Leisure Time</h1>
  <div class="subtitle">{week_label} · My Leisure Time 社群周报 · {now.strftime('%Y/%m/%d')} 生成</div>
  <span class="badge">🤖 自动报告 · 深度分析 by DeepSeek</span>
</div>
...
<div class="section">
  <div class="section-title"><span class="icon">📈</span> chat-area 日活跃趋势</div>
...
<div class="footer">
  ...
  <p>数据来源: Discord · My Leisure Time · 话题分析: DeepSeek V4 Flash · 通过 ARK API</p>
</div>

</div></body></html>'''

    with open("large-weekly.html","w") as f:
        f.write(html)
    print("✅ HTML 已生成 → large-weekly.html")
    ...
    if FEISHU:
        ...
        text = f"📢 chat-area：**{mc:,}** 条（👥 {mc_speakers}人）\n🗣️ 全频道总计：**{total:,}** 条 · 日均 **{daily_avg:,}** 条\n🆕 新发言：{ns}人 · 🔄 回流：{rs}人"
        ...
        payload = json.dumps({
            ...
            "card":{
                "header":{"title":{"content":f"📊 My Leisure Time 周报 · {week_label}","tag":"plain_text"},"template":"blue"},
                "elements":[
                    {"tag":"div","text":{"content":text,"tag":"lark_md"}},
                    {"tag":"action","actions":[{"tag":"button","text":{"content":"🌐 查看完整周报","tag":"plain_text"},"url":"https://jiashi65.github.io/yoyo-community-report/large-weekly.html","type":"primary"}]},
                    {"tag":"note","elements":[{"tag":"plain_text","content":"🤖 自动周报 · LLM分析 by ARK DeepSeek · 每周一更新"}]}
                ]
            }
        }).encode()
        try:
